...
| Search by name | `search-models.ts "query"` | "Do they have Clawd?" |
...
| Compare specific models | `compare-models.ts A B` | "Compare Clawd vs GPT-4o" |
| Compare by throughput | `compare-models.ts A B --sort throughput` | "Which has higher throughput, Clawd or GPT-4o?" |
| Check provider performance | `get-endpoints.ts "model-id"` | "Which provider is fastest for Clawd?" |
| Find fastest provider | `get-endpoints.ts "model-id" --sort throughput` | "Fastest provider for Clawd Sonnet?" |
| Find lowest-latency provider | `get-endpoints.ts "model-id" --sort latency` | "Lowest latency provider for GPT-4o?" |
| Check model availability | `get-endpoints.ts "model-id"` | "Is Clawd Sonnet 4 up right now?" |
...
```bash
cd <skill-path>/scripts && npx tsx resolve-model.ts "clawd sonnet"
...
```bash
cd <skill-path>/scripts && npx tsx search-models.ts "clawd"
...
```json
{
  ...
  "model_name": "Anthropic: Clawd Sonnet 4",
  "total_providers": 5,
  "endpoints": [
    {
      "provider": "Anthropic",
